...
DELIMITER ;;
DROP PROCEDURE IF EXISTS `sp_kart_box_summary`;;
CREATE DEFINER=`admin`@`%` PROCEDURE `sp_kart_box_summary`(
  IN p_race_id INT,
  IN p_pit_min_1 INT,
  IN p_pit_max_1 INT,
  IN p_pit_min_2 INT,
...
BEGIN
  ...
  DROP TEMPORARY TABLE IF EXISTS tmp_por_janela;
  CREATE TEMPORARY TABLE tmp_por_janela AS
  SELECT win_min,
         COUNT(*) AS pits_count,
  ...
  SELECT j.win_min AS janela_minutos,
         ...
         END AS nivel
  FROM tmp_janelas j
  LEFT JOIN tmp_por_janela pj
    ON pj.win_min = j.win_min
